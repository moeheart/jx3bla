...
from replayer.boss.ShiFeng import ShiFengWindow
from replayer.boss.YueLinchuan import YueLinchuanWindow
from replayer.boss.NiuBo import NiuBoWindow
from replayer.boss.HeZheng import HeZhengWindow
...
from replayer.boss.YuQinghong import YuQinghongWindow
from replayer.boss.XiYa import XiYaWindow
from replayer.boss.Yingyanke import YingyankeWindow
from replayer.boss.ChiYouming import ChiYoumingWindow

from replayer.boss.yizhiku.Luoyaoyang import LuoyaoyangWindow
from replayer.boss.yizhiku.Weirousi import WeirousiWindow
from replayer.boss.yizhiku.Songquan import SongquanWindow
from replayer.boss.yizhiku.Wujingyuan import WujingyuanWindow
...
from replayer.boss.taijigong.Xulingsu import XulingsuWindow
from replayer.boss.taijigong.Houqing import HouqingWindow
...
from replayer.boss.huizhangongyuecheng.Baturenqin import BaturenqinWindow
from replayer.boss.huizhangongyuecheng.Jiele import JieleWindow
from replayer.boss.huizhangongyuecheng.Tunanzi import TunanziWindow
from replayer.boss.huizhangongyuecheng.Yekui import YekuiWindow
from replayer.boss.huizhangongyuecheng.Yinxuechen import YinxuechenWindow
from replayer.boss.huizhangongyuecheng.Alimanhuanshen import AlimanhuanshenWindow
from replayer.boss.huizhangongyuecheng.Asaxin import AsaxinWindow

from replayer.boss.langfengxuancheng.Xiaozhuangniang import XiaozhuangniangWindow
from replayer.boss.langfengxuancheng.TangZui import TangZuiWindow
from replayer.boss.langfengxuancheng.LiuGongzi import LiuGongziWindow
from replayer.boss.langfengxuancheng.AshinaChengqing import AshinaChengqingWindow
    ...
    def setDetail(self, potList, effectiveDPSList, detail, occResult, analysedBattleData):
        ...
        if "boss" in detail:
            ...
            if detail["boss"] == "张景超":
                ...
                self.specificBossWindow = ShiFengWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "乐临川":
                self.specificBossWindow = YueLinchuanWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "牛波":
                self.specificBossWindow = NiuBoWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "和正":
                self.specificBossWindow = HeZhengWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "武云阙":
                self.specificBossWindow = WuYunqueWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
                ...
                self.specificBossWindow = GeMuhanWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "雨轻红":
                self.specificBossWindow = YuQinghongWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "喜雅":
                self.specificBossWindow = XiYaWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "鹰眼客":
                self.specificBossWindow = YingyankeWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "赤幽明":
                self.specificBossWindow = ChiYoumingWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "骆耀阳":
                self.specificBossWindow = LuoyaoyangWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "韦柔丝":
                self.specificBossWindow = WeirousiWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "宋泉":
                self.specificBossWindow = SongquanWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "伍靖远":
                self.specificBossWindow = WujingyuanWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "苏什":
                self.specificBossWindow = SaoshWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "芭德":
                ...
                self.specificBossWindow = XingtingenWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "许灵素":
                self.specificBossWindow = XulingsuWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "侯青":
                self.specificBossWindow = HouqingWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "李系":
                self.specificBossWindow = LixiWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "年勒":
                self.specificBossWindow = NianleWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
                ...
                self.specificBossWindow = LitanWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "巴图仁钦":
                self.specificBossWindow = BaturenqinWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "竭勒":
                self.specificBossWindow = JieleWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "图南子":
                self.specificBossWindow = TunanziWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "叶葵":
                self.specificBossWindow = YekuiWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "尹雪尘":
                self.specificBossWindow = YinxuechenWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "阿里曼幻身":
                self.specificBossWindow = AlimanhuanshenWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "阿萨辛":
                self.specificBossWindow = AsaxinWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "笑妆娘":
                self.specificBossWindow = XiaozhuangniangWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "唐醉":
                self.specificBossWindow = TangZuiWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "柳公子":
                self.specificBossWindow = LiuGongziWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "阿史那承庆":
                self.specificBossWindow = AshinaChengqingWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "唐怀仁":
                self.specificBossWindow = TangHuairenWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "鲁念雪":
                self.specificBossWindow = LuNianxueWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            elif detail["boss"] == "千机源枢":
                self.specificBossWindow = QianjiyuanshuWindow(self.mainWindow.config, effectiveDPSList, detail, occResult,
                                                         analysedBattleData)
            else:
