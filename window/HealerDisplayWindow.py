...
class HealerDisplayWindow(Window):
    ...
    def OpenInWeb(self):
        ...
        打开网页版的复盘界面.
        '''
        url = get_logs_url('/%s/%d' % (self.occ, self.result["overall"]["shortID"]))
        webbrowser.open(url)
    ...
    def renderRate(self):
        ...
        if "review" in self.result:
            ...
            tk.Label(frame8, text="本模块所用的内容基于早期版本，").place(x=20, y=110)
            tk.Label(frame8, text="可能已经失效，仅供参考！").place(x=20, y=130)
