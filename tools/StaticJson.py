...
REVIEW_JSON = {
  ...
  "content": [
    ...
    },
    {
      ...
      "threshold": [95, 90, 0, 0],
      "descCondition": [1, 2, 3],
      "desc": "药奶的中和机制需要药性在温性与寒性之间互相抵消，而药性超过+5或-5的部分不会被累加，你需要调整技能的施放方式以尽量避免这种情况。",
      "addCondition": [0, 1, 2, 3],
    ...
    },
    {
      ...
      "maxLayer": "最高的七情层数",
      "rank": "天网排名",
      "rate": "等于min(100, 200-20*`maxlayer`)",
      "threshold": [100, 50, 0, 0],
      "descCondition": [1, 2, 3],
      "desc": "`七情和合`原本是优秀的爆发技能，但多次叠加`七情`会使自身叠加显著的治疗降低效果，因此需要尝试在合适的时机消除`七情`。",
      "addCondition": [1, 2, 3],
    ...
    },
    {
      ...
      "numWaste": "浪费的数量",
      "numAll": "总数量",
      "rate": "等于1 - `numWaste` / `numAll`",
      "threshold": [90, 0, 0, 0],
      "descCondition": [1, 2, 3],
      "desc": "如果你点了`素柯`，不要忘记触发它。",
      "addCondition": [1, 2, 3],
      "add": "你浪费了`numWaste`次`素柯`。"
    },
    {
      "code": 401,
      ...
      "threshold": [75, 50, 25, 0],
      "descCondition": [1, 2, 3],
      "desc": "根据流派不同，`冰蚕牵丝`或`醉舞九天`之一是你的主要填充技能，你需要保证其触发次数从而提供稳定的治疗。（冰蚕yyds，醉舞已死）",
      "addCondition": [0, 1, 2, 3],
    ...
    },
    {
      ...
      "threshold": [30, 0, 0, 0],
      "descCondition": [1, 2, 3],
      "desc": "放不放锅是一回事，吃不吃是另一回事。你可以使用警长自带的功能来出警不吃锅的队友，而警长也可以出警不放锅的你。"
    },
    {
      "code": 404,
      "title": "使用`迷仙引梦`",
      ...
      "threshold": [50, 0, 0, 0],
      "descCondition": [1, 2, 3],
      "desc": "`迷仙引梦`的治疗收益非常高，而且具有回蓝效果，应该好了就放，尽量不要让其空转。"
    },
    {
      "code": 405,
      "title": "使用`蚕引`层数",
    ...
    },
    {
      ...
      "threshold": [85, 50, 0, 0],
      "descCondition": [1, 2, 3],
      "desc": "你需要在`回雪飘摇`第二跳之后直接使用gcd技能，而不是等倒读条结束，这样每6跳回雪飘摇中你就多了一个施展gcd技能的机会。",
      "addCondition": [1, 2, 3],
      "add": "你总共运功了`time`次`回雪飘摇`，但其中只正确中断了`perfectTime`次。"
    },
    {
      "code": 504,
      "title": "不要重复施放`上元点鬟`",
