...
REVIEW_JSON = {
  ...
  "content": [
    ...
    },
    {
      "code": 21,
      "title": "保持gcd不要空转",
      "note": "这是DPS心法的gcd提示，其与治疗心法逻辑不同。",
      "cover": "战斗效率",
      "rate": "等于`cover`",
      "threshold": [95, 90, 85, 0],
      "descCondition": [1, 2, 3],
      "desc": "保持使用技能是战斗的基础。只要你持续处于读条或者转gcd的状态，就一定比站在原地发呆好。记得为可能产生的移动做好准备，在移动时也可以使用瞬发技能。只要剑三还没有施法队列机制，善于利用辅助宏、连发、加速器等操作就可以极大提升gcd效率。"
    },
    {
      "code": 90,
      "title": "敬请期待",
    ...
    },
    {
      "code": 1000,
      "title": "花间心法描述",
      "desc": "花间游天下第一！花间的循环中有随机成分、4种持续伤害效果和2种buff，对技能的先后顺序安排有很多的要求。这个工具可以帮你快速找到手法中的明显缺陷；但在实战处理上，需要基于这些基础要求来灵活调整。"
    },
    {
      "code": 1001,
      "title": "优先使用`兰摧玉折`",
      "failTime": "打错技能的时间点，数组形式",
      "num": "打错技能的次数",
      "sum": "总共打了多少个技能",
      "rate": "等于1 - `num` / `sum`",
      "threshold": [100, 0, 0, 0],
      "descCondition": [1, 2, 3],
      "desc": "由于`倚天`的效果，在目标有`兰摧玉折`时，你对其造成的伤害会有加成，因此在目标身上没有`兰摧玉折`buff时，应该优先使用之。这种情况通常发生在`玉石俱焚`结算后，或`阳明指`随机吞噬后。少数例外情况包括：乱洒循环、橙武buff处理、卡布散buff。",
      "addCondition": [1, 2, 3],
      "add": "有`num`次技能本应被替换为`兰摧玉折`，分别为：`failTime`。"
    },
    {
      "code": 1002,
      "title": "使`玉石俱焚`覆盖到全部的持续伤害效果",
      "failTime": "打错技能的时间点，数组形式",
      "num": "打错技能的次数",
      "sum": "总共打了多少个技能",
      "rate": "等于1 - `num` / `sum`",
      "threshold": [100, 98, 95, 0],
      "descCondition": [1, 2, 3],
      "desc": "在使用`玉石俱焚`时保证目标拥有`商阳指``钟林毓秀``兰摧玉折``快雪时晴`全部四种持续伤害效果，这样才能触发最高的破招伤害。如果DOT被吞噬或者因为超时消失，你应该先补齐。少数例外情况包括：橙武buff处理。",
      "addCondition": [1, 2, 3],
      "add": "有`num`次`玉石俱焚`的结算时DOT数量不足，分别为：`failTime`。"
    },
    {
      "code": 1003,
      "title": "在`芙蓉并蒂`后立刻结算持续伤害",
      "failTime": "打错技能的时间点，数组形式",
      "num": "打错技能的次数",
      "sum": "总共打了多少个技能",
      "rate": "等于1 - `num` / `sum`",
      "threshold": [100, 0, 0, 0],
      "descCondition": [1, 2, 3],
      "desc": "在使用`芙蓉并蒂`后应该立刻使用`玉石俱焚`，否则会因为跳数不足而损失破招伤害与玉石的加成伤害。少数例外情况包括：卡布散buff。",
      "addCondition": [1, 2, 3],
      "add": "有`num`次`芙蓉并蒂`后没有立刻使用`玉石俱焚`，分别为：`failTime`。"
    },
    {
      "code": 1004,
      "title": "尽可能使用`芙蓉并蒂`",
      ...
      "threshold": [100, 98, 95, 0],
      "descCondition": [1, 2, 3],
      "desc": "在`芙蓉并蒂`调息完成时应当尽可能使用，从而增加破招伤害和触发`青冠`的大量增伤。少数例外情况包括：橙武buff处理。",
      "addCondition": [1, 2, 3],
      "add": "有`num`次`玉石俱焚`之前的应当使用`芙蓉并蒂`但未使用，分别为：`failTime`。"
    },
    {
      "code": 1005,
      "title": "使用瞬发`阳明指`尝试触发`踏歌`",
      "failTime": "打错技能的时间点，数组形式",
      "num": "打错技能的次数",
      "sum": "总共打了多少个技能",
      "rate": "等于1 - `num` / `sum`",
      "threshold": [100, 98, 95, 0],
      "descCondition": [1, 2, 3],
      "desc": "每个`阳明指`都应该尝试满足`踏歌`的条件，也即消耗墨意打出，并且目标上至少有一个持续伤害效果。避免因为墨意不足打出读条的`阳明指`，或者对没有毒的目标施放`阳明指`。",
      "addCondition": [1, 2, 3],
      "add": "有`num`次`阳明指`无法满足`踏歌`的触发条件，分别为：`failTime`。"
    },
    {
      "code": 1006,
      "title": "使用瞬发`阳明指`尝试触发`雪中行`",
      "failTime": "打错技能的时间点，数组形式",
      "num": "打错技能的次数",
      "sum": "总共打了多少个技能",
      "rate": "等于1 - `num` / `sum`",
      "threshold": [100, 95, 0, 0],
      "descCondition": [1, 2, 3],
      "desc": "`雪中行`奇穴将花间循环分为`逢雪`和`溅玉`两部分，只有在`逢雪`buff为0或1层时，`阳明指`才能增加`逢雪`的计数。避免在2层`逢雪`或`溅玉`期间使用`阳明指`，这时应该打快雪或者补毒。",
      "addCondition": [1, 2, 3],
      "add": "有`num`次`阳明指`无法满足`雪中行`的触发条件，分别为：`failTime`。"
    },
    {
      "code": 1007,
      "title": "在`逢雪`气劲层数最大时使用`快雪时晴`",
      "failTime": "打错技能的时间点，数组形式",
      "num": "打错技能的次数",
      "sum": "总共打了多少个技能",
      "rate": "等于1 - `num` / `sum`",
      "threshold": [100, 98, 95, 0],
      "descCondition": [1, 2, 3],
      "desc": "`快雪时晴`会尽可能将`逢雪`气劲转化为`溅玉`气劲以获得大量输出增益。如果`逢雪`气劲不足2层时施展`快雪时晴`，就无法获得满额的增益。",
      "addCondition": [1, 2, 3],
      "add": "有`num`次`快雪时晴`没有在2层`逢雪`时打出，分别为：`failTime`。"
    },
    {
      "code": 1008,
      "title": "打满`快雪时晴`的跳数",
      ...
      "threshold": [100, 0, 0, 0],
      "descCondition": [1, 2, 3],
      "desc": "完整运功`快雪时晴`可以获得6层持续伤害效果，这是DOT伤害中最高的。未完整运功会损失持续伤害的层数，需要避免。少数例外情况包括：卡布散buff。",
      "addCondition": [1, 2, 3],
      "add": "有`num`次`快雪时晴`没有完整运功导致损失了跳数，分别为：`failTime`。"
    },
    {
      "code": 1009,
      "title": "避免重复施展`快雪时晴`",
      ...
      "threshold": [100, 0, 0, 0],
      "descCondition": [1, 2, 3],
      "desc": "`快雪时晴`主要的意义在于添加持续伤害效果，其技能本身的伤害小于循环平均值。避免在`快雪时晴`buff已经达到6层时再打出`快雪时晴`。少数例外情况包括：群攻。",
      "addCondition": [1, 2, 3],
      "add": "有`num`次`快雪时晴`重复施展导致DOT溢出，分别为：`failTime`。"
    },
    {
      "code": 1010,
      "title": "避免重复施展持续伤害效果",
      ...
      "threshold": [100, 98, 95, 0],
      "descCondition": [1, 2, 3],
      "desc": "避免在目标已经有`商阳指``钟林毓秀``兰摧玉折`的情况下再次施展相同的持续伤害效果，这会显著降低循环质量。",
      "addCondition": [1, 2, 3],
      "add": "有`num`次持续伤害效果重复施展，分别为：`failTime`。"
    },
    {
      "code": 1011,
      "title": "将持续伤害技能延后到`溅玉`期间",
      "failTime": "打错技能的时间点，数组形式",
      "num": "打错技能的次数",
      "sum": "总共打了多少个技能",
      "rate": "等于1 - `num` / `sum`",
      "threshold": [100, 95, 0, 0],
      "descCondition": [1, 2, 3],
      "desc": "除非循环里有`芙蓉并蒂`，否则尽可能让`兰摧玉折`以外的持续伤害技能在`溅玉`期间打出，这样可以最大化利用`溅玉`的基础攻击加成。",
      "addCondition": [1, 2, 3],
      "add": "有`num`次`DOT可以延后，分别为：`failTime`。"
    }
  ]
}
