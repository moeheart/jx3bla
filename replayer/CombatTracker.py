...
class BoostCounter():
    ...
    def removeTargetBoost(self, targetid, id, time):
        '''
        移除一个目标增益.
        params:
        - targetid: 目标ID.
    ...
    def addTargetBoost(self, targetid, id, effect, source, stack, time):
        '''
        添加一个目标增益.
        params:
        - targetid: 目标ID.
...
class DpsCastRecorder(StatRecorder):
    ...
    def export(self, time, info, player, boostCounter, badPeriodLog):
        ...
        for source in self.source:
            ...
            cover = -1
            sourceBuff = source[2:]
            if boostCounter is not None and player in boostCounter and sourceBuff in boostCounter[player].boostCover:
...
class CombatTracker():
    ...
    def checkRemoveBuff(self, time):
        ...
        toRemove = []
        for key in self.boostRemove:
            if self.boostRemove[key]["time"] <= time:
                # 执行移除
                for player in self.boostCounter:
                    self.boostCounter[player].removeTargetBoost(self.boostRemove[key]["target"], self.boostRemove[key]["boost"], time)
                toRemove.append(key)
        for key in toRemove:
            del self.boostRemove[key]
    ...
    def recordShout(self, event):
        ...
        if event.content in ['"可恶...这是怎么了？"', '"可惡… 這是怎麼了？"']:
            bossID = []
            for id in self.info.npc:
                if self.info.getName(id) in ["岑伤", "岑傷"]:
                    bossID.append(id)
            for bossid in bossID:
                effect_id = "2,26734,1"
                boostValue = BOOST_DICT[effect_id]
