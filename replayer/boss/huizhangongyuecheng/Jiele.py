...
class JieleWindow(SpecificBossWindow):
    '''
    竭勒的定制复盘窗口类。
    '''
    ...
    def loadWindow(self):
        ...
        使用tkinter绘制详细复盘窗口。
        '''
        self.constructWindow("竭勒", "1200x800")
        window = self.window
...
class JieleReplayer(SpecificReplayerPro):
    ...
    def analyseSecondStage(self, event):
        ...
        if event.dataType == "Skill":
            if event.target in self.bld.info.player:
                ...
                if event.caster in self.bld.info.player and event.caster in self.statDict:
                    ...
                    if event.target in self.bld.info.npc:
                        if self.bld.info.getName(event.target) in ["竭勒"]:
            ...
            if event.id in self.bld.info.npc and self.bld.info.npc[event.id].name in ["竭勒宝箱", "竭勒寶箱"]:
                self.win = 1
                self.bh.setBadPeriod(event.time, self.finalTime, True, True)
            ...
            if event.id in self.bld.info.npc and self.bld.info.getName(event.id) in ["竭勒"]:
                self.win = 1
                self.bh.setBadPeriod(event.time, self.finalTime, True, True)
    ...
    def initBattle(self):
        ...
        self.bhBlackList.extend(["s41825", "b31501", "s41857",  #苍焰流虹
                                 "s41827",  # 离弦
                                 "s42092", "b31903",  # 破阵斩
                                 "s41812",  # 苍原弧月
                                 "b31768",  # 阻力
        ...
        self.bhInfo = {"c41826": ["2026", "#ff0000", 5000],  # 苍焰流虹
                       "c41828": ["2024", "#ff7700", 4000],  # 离弦
                       "c42204": ["347", "#00ff00", 5000],  # 破阵斩
                       "c41839": ["2119", "#ff0077", 4000],  # 苍原弧月
                       "c41824": ["2120", "#ff7777", 1750],  # 苍原弧月
                       }
