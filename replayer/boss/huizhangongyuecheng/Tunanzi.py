...
class TunanziWindow(SpecificBossWindow):
    '''
    图南子的定制复盘窗口类。
    '''
    ...
    def loadWindow(self):
        ...
        使用tkinter绘制详细复盘窗口。
        '''
        self.constructWindow("图南子", "1200x800")
        window = self.window
...
class TunanziReplayer(SpecificReplayerPro):
    ...
    def analyseSecondStage(self, event):
        ...
        if event.dataType == "Skill":
            if event.target in self.bld.info.player:
                ...
                if event.id == "41786":
                    self.bh.setCritPeriod(event.time, event.time + 30000, False, True)

            else:
                if event.caster in self.bld.info.player and event.caster in self.statDict:
                    # self.stat[event.caster][2] += event.damageEff
                    if event.target in self.bld.info.npc:
                        if self.bld.info.getName(event.target) in ["图南子"]:
            ...
            if event.content in ['"哼……"', '"擅闖皇宮禁地者死！"']:
            ...
            if event.id in self.bld.info.npc and self.bld.info.npc[event.id].name in ["图南子宝箱", "圖南子寶箱"]:
                self.win = 1
                self.bh.setBadPeriod(event.time, self.finalTime, True, True)
            ...
            pass
            # if event.id in self.bld.info.npc and self.bld.info.getName(event.id) in ["图南子"]:
            #     self.win = 1
            #     self.bh.setBadPeriod(event.time, self.finalTime, True, True)
    ...
    def initBattle(self):
        ...
        self.bhBlackList.extend(["s41777", "b31462",  # 普攻
                                 "s41778", "s41780", "s41781", "b31773", "b31468",  # 击浪式
        ...
        self.bhInfo = {"c41779": ["16370", "#ff0000", 5000],  # 击浪式
