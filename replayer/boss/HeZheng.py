...
class HeZhengWindow(SpecificBossWindow):
    '''
    和正的定制复盘窗口类。
    '''
    ...
    def loadWindow(self):
        ...
        使用tkinter绘制详细复盘窗口。
        '''
        self.constructWindow("和正", "1200x800")
        window = self.window
...
class HeZhengReplayer(SpecificReplayerPro):
            ...
            if event.target in self.bld.info.player:
                ...
                if event.caster in self.bld.info.player and event.caster in self.statDict:
                    ...
                    if event.target in self.bld.info.npc:
                        if self.bld.info.getName(event.target) in ["和正"]:
