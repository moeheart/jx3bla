...
- 修复有时覆盖率会超过100%的问题。
  ...
  - 看一下覆盖率本身的代码
- (TODO)支持西津渡1-6的副本细节复盘。
  - 1号加入细节统计，记录两次分别的球承伤和内场承伤。
  - 2号加入细节统计，记录两个BOSS分别的承伤，以及射箭记录。
  - 3号加入细节统计，记录每个阶段的承伤。
  - 4号加入细节统计，记录承伤，及接罐子和扶摇的次数。
  - 6号的本体输出、小怪输出与牢笼点名。
  - 6号尝试展开显示控怪的细节。
  - 6号的驱散、打断，镜影的反向输出，怒阶段处理错误。
  - 查一下洗髓的大狮子吼、图穷匕见、奇门飞宫
- (TODO)调整rDPS算法细节。
  - 把雷和袖气加入低等级增益，尝试通过团队配置自动分析。
  - (TODO)查一下花间的专属增益buff，包括快雪、无视防御、涓流。
  - 加入BOSS的易伤逻辑，把易伤算作环境增益独立出来 
  - (TODO)重置yx6号的统计。
  - 在box上加文档
  - 修复治疗心法rdps不正确的问题。
- (TODO)加一些五奶复盘关心的数据
  - (TODO)药奶复盘中加一个治疗吸收的统计
  - (TODO)奶花复盘记录阳明指
...
- (TODO)加一些副本功能
- (TODO)加一个dps复盘
- 在act中，对rdps中的增益来源现在可以显示覆盖率。
- 在act中，对rdps重新定义了雷、袖气、BOSS自带易伤的判断方式，并(TODO)重置了6号的rdps统计。

## 修复

- 修复非BOSS的目标混入伤害统计时，会导致统计失败的问题。
- 修复秋肃之类的技能在不按预期的方式叠加时，会使覆盖率超过100%的问题。现在理论上所有的覆盖率都不会超过其层数。
- (TODO)分山不打特定技能会判断失败
