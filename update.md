...
- 支持洛阳之战 1-5 的专属复盘，展示首领阶段、关键招式、点名和主次目标伤害，并按通关信号截断尾录。
- 启用 50 级装备、心法面板与 rDPS/mrDPS 计算，修正增益实例、来源归属和分系伤害处理。
- 更新增益、技能名称及装备底表至体验服 1.6.0.9506（2026-09-20 解包核验）；秋肃、左旋右转、庄周梦每层均为 7 点无双等级，飘黄为 8 点。
- 同步神兵·无双和新装备附魔数据；旧赛季继续使用独立底表。
- 正式安装包包含洛阳之战本地属性计算所需资源，无需另行放置装备表。
- 实战验证范围为 25 人普通洛阳之战；10 人、英雄及治疗心法新技改评分仍需对应样本验证。

# 8.16.0-beta.1

## 苍生铸世体验服

- 接入洛阳之战 1-5 的专属复盘：突利和顺、田承嗣、伊曼双子、阿史那承庆、史朝义。
- 记录首领关键招式、阶段、点名、主次目标伤害及有日志依据的机制事件；按明确通关信号截断尾录。
- 修复史朝义记录被命名为李复、煞将或空名时的识别和尝试次数归并；与阆风悬城同名首领按地图区分。
- 根据 2026-09-15 的体验服 1.6.0.9503 解包更新独立增益底表，修复重复属性覆盖、气劲等级及分系减伤/吸收识别；保留旧赛季底表。
- 支持 GBK/UTF-8 JCL，以及洛阳之战不连续的地图编号 828、835、836。
- 50 级面板与 rDPS 尚待完整校准，当前明确显示“待校准”，使用日志实测伤害/治疗展示，并暂停未校准结果上传。治疗心法专属技改与评分不属于本次完整适配范围。
- 本轮实战验收范围为 25 人普通体服日志；10 人、英雄和后续技改仍需对应样本验证。
...
- 修复一个在药奶枯木复盘中可能导致崩溃的问题。

# 8.14.9

## 修复

- 修复一个可能会使阿萨辛P2平血查锅失败的问题。

# 8.14.8

## 功能

- 增加阿里曼幻身和阿萨辛的时间轴记录。由于这会识别出一些战斗中的无效时间，会导致rdps统计发生变化。
- 增加阿萨辛的详细复盘，包括每个阶段的伤害、P2的平血复盘。
- 增加阿萨辛的trivia统计。如果在设置中开启，会将战斗的摘要独立存放，用于进一步的分析。

# 8.14.7

## 功能

- 增加阿萨辛的通关判定。（具体复盘项目后续或许会更新）
...
- 修复部分悟心法和无相楼不能正常识别，且导致复盘卡死的问题。
- 正确统计爆发朝。（其实是更新之后自己就好了）
- 数据更新到体服二改之后的数值（主要影响各种无双增益）
- 修复数值统计中部分情况下配色不正确的问题。
...
- 更新会战弓月城1-5的数据。
- (TODO)增加排行榜功能，在设置面板中设置团名，即可参与排行榜，记录包括挑战本在内的通关时间。
- 从会战弓月城开始，可以正确识别部分穿透伤害不参与减伤计算，得到更准确的rhps。
- 略微修改了飘黄的统计方式，现在还可以看到飘黄的覆盖率和层数。

# 8.13.7

## 功能

- 更新buff表，以处理药奶技改后飘黄的rdps.
...
- 更新技改的装备表。(之前的版本会使一些影响无双的增益数值不正常，部分为在线更新)
...
- 修复读取装备表时如果有玩家存在空的装备栏，会导致复盘崩溃的问题。
...
- 实现服务器的装备存档，在游戏中获取装备失败的时候也尝试从服务器获取。
