...
- (TODO)优化配装获取属性的过程
  - (TODO)装备表重新读取
  - (TODO)用新版本的属性计算重构之前的配装器
  
# 修复

- (TODO)修复有时会把般若之类的buff算到目标身上导致莫名其妙出现在rdps里的问题。（还没有定位到）
...
- (TODO)底层扩展
  - 更新120级的各种系数，并且给出一个专门用来维护系数的方法.
    - 整理所有属性相关的内容
    - 实现加速
  - 修复除以0导致的代码边界问题
    - 逐个替换
  - (TODO)修复所有手动实现的BOSS名称信息，全部改为自动检查.
    - 修复之前的流程
      - 检查所有文件
      - 服务器修复
        - (TODO)部署
  ...
  - 颜色统一获取
    - 把所有的颜色合并获取
    - (TODO)刀宗
  - (TODO)修改界面的形状，使其不需要翻页就能展示所有数据
    - (TODO)扩展HPS的显示窗口，显示分目标的数据
    - (TODO)扩展nDPS的显示窗口，显示分目标的数据
    - (TODO)扩展rDPS的显示窗口，显示DPS增益转移的来源与去向
...
- (TODO)实现rdps算法。rdps可以将所有增益转移给来源，以得到一个更能体现个人水平和实际贡献的数值，从而进行排名。
- 移除部分废弃设定，简化界面。将logs入口移动到主界面中。
- 实装pt西津渡1-6的判定并支持复盘。(TODO)放生队友的判定
- (TODO)实装五奶的技改并调整部分统计项目。
...
- (TODO)修复rhps之类的记录没有对应数据时会使复盘崩溃的问题。
- (TODO)修复药奶秒开关千枝会影响中和覆盖的问题。
- (TODO)对入库的步骤加锁，避免出现数据异常的问题。
