...
- 扩展HPS的定义，更新不同HPS的指标。现在可以通过rHPS来更准确地衡量治疗职业对团队血量的贡献。
  ...
  - 计算rHPS
    ...
    - 按血量状态推测的结果分配rHPS
- (TODO)重写HPS统计模块
  - (TODO)在战斗统计中增加一个单独的HPS展示界面
    - 框架
    - 条形界面实现
    - 表格界面实现
    - 添加说明
  - 在五奶复盘的展示中换成新的HPS统计
  - 在五奶复盘中复制一份HPS统计的存储位置以记录排名
  - (TODO)在网页版中增加对应的实现
    - (TODO)修改数据库结构
    - (TODO)修改服务端请求流程，让多个复盘共享一个数据
    - (TODO)为每个复盘增加"同场战斗中其它角色复盘编号"的接口
...
- (TODO)上传数据已存在时，若上传设置为公开，则覆盖数据库中是否公开的选项。
- 修复奶秀的沐风统计错误的问题。
...
- 修复奶毒蛊惑被算成gcd技能的问题。
- (TODO)修复有时jx3dat的装备统计错误导致复盘崩溃的问题。
