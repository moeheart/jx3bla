...
(TODO)更新6号首领的详细查锅，至此达摩洞1-6的详细查锅已经全部更新完毕！

(TODO)天网系统加入指定查询功能，可以查询某个区服指定的某些玩家的记录。

(TODO)增加*复盘*按钮。现在即使关掉了复盘界面，也可以点击此按钮来打开最后一次记录了。

(TODO)移除复盘模式下的演员复盘图片。现在在复盘模式下也会以原本实时模式的方式来解析，通过*复盘*即可查看。

(TODO)将*分锅结果*界面调整为*总结*。在其中可以像之前一样看到每个人的锅数。在之后的版本中，可以通过这一页面直接出警。

(TODO)在*总结*页面中显示所有的战斗记录名称。可以通过点击按钮来快速切换到对应的战斗复盘。

(TODO)在6号的战斗结束时，会提醒玩家点击复盘模式，来复盘奶歌的记录。

##优化

(TODO)在实时复盘崩溃时，加入一个异常处理，保证在一个复盘崩溃后可以复盘其它的记录。
